...
jQuery(document).ready(function ($) {
    ...
    $('.header__nav > li.dropdown:not(:last-child) > a').html(function () {
        var text = $(this).text().split(' ');
        var last = text.pop();
        return text.join(" ") + (text.length > 0 ? '<span class="header__arrow"></span><br>' + last + '' : last);
    });
    ...
    var firstMemberWrapper = $('.solutions__member--first .solutions__member-item'),
        secondMemberWrapper = $('.solutions__member--second .solutions__member-item');

    firstMemberWrapper.html($('.solutions__member-item[data-number=' + 1 + ']').html());
    secondMemberWrapper.html($('.solutions__member-item[data-number=' + 2 + ']').html());
    ...
    const closeAllSibling = (el) => {
        ...
        while (node) {
            if (node !== el && node.nodeType === Node.ELEMENT_NODE && node.classList.contains('col-lg-8')) {
